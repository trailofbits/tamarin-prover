...
collectPaths ctxt prf = M.fromList $ zip [0..] $ map (\p -> (p, getMethods p)) $ go prf
  where
    ...
    getMethods path = fromMaybe (error "illegal path") $ do
      sys <- prf `atPath` path >>= psInfo . root >>= viewSystem
...
solve pathIdx methodIdx prf =
  ...
  sys <- maybeREPL "illegal path" (iPrf `atPath` path >>= psInfo . root >>= viewSystem)
...
systemAt pathIdx prf =
  ...
  in do
    ...
    maybeREPL "illegal path" (iPrf `atPath` path >>= psInfo . root >>= viewSystem)
